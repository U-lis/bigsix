 ...
 * 하단 4탭을 경로에서 계산한다.
 *
 * 이전에는 `+layout.svelte` 안에 `<a href="/">오늘</a>` 줄이 마크업으로 굳어
 * 있었다. 소속과 순서와 라벨 셋을 마크업에 섞어 두면 탭이 바뀌거나 하나가 늘
 * 때 마크업과 활성 판정을 두 곳에서 손으로 맞춰야 한다 — cube-study 가 겪은
 * 것과 같은 함정이다.
 *
 * bigsix 의 탭은 4개 고정이고 계층도 없어서 큐브의 `SIBLING_GROUPS` 트리 계산
 ...
 * 라우트 id 로 부모 형제군을 찾는 트리 함수인데, bigsix 는 트리가 없어서 4개
 * 상수를 단순히 훑고 활성만 표시한다.
 */
...
/**
 * 4탭. 순서는 이 배열이 정한다 (FR-23.1: 오늘 · 프로그램 · 단계 · 기록).
 *
