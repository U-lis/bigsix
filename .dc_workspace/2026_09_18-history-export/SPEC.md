...
| H-8 | 동작 설명 | 운동 중 자세·방법을 알 수 없어 불편했다 → **오늘 화면 운동 카드**에서 그 단계의 동작 설명을 본다 (FR-30). 본문은 지금 데이터에 있는 요약을 먼저 쓰고, 사용자가 사진 자료를 다시 주면 그때 보강한다 |
...
### FR-30: 동작 설명 노출 (오늘 화면)

운동 중에 "이 자세를 어떻게 하는 것인가" 를 앱이 답하지 못했다 (2026-09-21 사용자 보고).
데이터는 이미 있다 — `src/lib/data/progressions.json` 의 60단계 전부에 `summary`(2~3줄, 평균 87자)가
들어 있고 `Step.summary: string[]` 로 도메인 타입에도 노출돼 있다 (`src/lib/domain/types.ts:35`).
`docs/MOVEMENTS.md` 가 이 데이터로 생성된 문서다. **화면에서 부르는 곳만 없다.**

- [ ] FR-30.1: 오늘 화면의 운동 카드에서 그 단계의 동작 설명을 본다. 기본은 접힌 상태이고 펼쳐서 읽는다.
      접기는 CSS 로 한다 — `{#if}` 로 DOM 에서 빼지 않는다 (`CLAUDE.md`).
- [ ] FR-30.2: 보여주는 것은 **실제로 수행하는 단계**(`performedStep`)의 설명이다. 다지기 세션이면
      이전 단계, 자유 운동이면 사용자가 고른 단계.
- [ ] FR-30.3: 함께 적는 것 — 단계명(한/영) · 단위(회/초) · `perSide` 면 좌우 고지(기존 `sideNote` 재사용) ·
      한국어판 쪽수(`Step.page`). 책을 펴서 확인할 수 있어야 한다.
- [ ] FR-30.4: `pairWith` 가 있는 단계(핸드스탠드 2단계 → 1단계)는 동반 단계의 설명도 함께 낸다.
- [ ] FR-30.5: 문구는 데이터를 **가공 없이** 낸다 (NFR-2). 격려·난이도 평가를 덧붙이지 않는다.
- [ ] FR-30.6: 설명 본문의 출처를 화면에 밝힌다 — "동작 설명은 책 원문이 아니라 자체 요약" (README 와 같은 문구).
- [ ] FR-30.7 (보강, 사진 자료 도착 시): `progressions.json` 의 `summary` 를 자세·호흡·흔한 실수 중심으로
      늘린다. **데이터만 고치고 화면 코드는 그대로다.** 늘어난 줄 수만큼 카드가 길어질 뿐이다.
      책 원문을 그대로 옮기지 않는다 (저작권 — README 「데이터 출처」).
...
- [ ] UI-11 동작 설명(FR-30)은 운동 카드 안에서 **접힌 채로 자리를 지킨다.** 펼침 상태는 기억하지 않는다
      (브라우저 상태에서 오지 않으므로 자리 예약 대상이 아니다 — CONVENTIONS 3.1). 훅은 `data-howto`,
      펼침 토글은 `data-howto-toggle`. 제목·본문이 카드의 **목표 아래**에 온다 — 목표가 시선의 첫 지점이라는
      기존 위계(FR-21)를 흔들지 않는다.
...
| # | 상황 | 요구되는 동작 |
|---|---|---|
| EC-71 | `summary` 가 빈 배열인 단계 | 설명 영역을 그리지 않는다. 빈 상자를 두지 않는다 |
| EC-72 | 다지기 세션 카드 | 이전 단계(`performedStep`)의 설명이 뜬다. 현재 단계 설명이 뜨면 안 된다 (FR-30.2) |
| EC-73 | `pairWith` 단계(핸드스탠드 2단계) | 두 단계의 설명이 모두 뜨고 어느 쪽이 어느 단계인지 구분된다 (FR-30.4) |
...
- 동작 **이미지·영상** — 이슈 #3. 이번엔 글 설명만 (FR-30)
- 단계 화면(`/steps`) · 별도 동작 화면에서의 설명 노출 — 이번엔 오늘 화면 카드만 (H-8)
