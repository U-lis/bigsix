...
// --- 유효한 body → 201, 재요청 → 200, createdAt 유지 · updatedAt 갱신 ---
{
	...
	try {
		...
		const sub = store.subscriptions['https://push.example/endpoint-a'];
...
// --- DELETE — 존재하지 않는 endpoint 도 204 ---
{
	...
	const { base, close } = await bootServer({ dataDir: dir });
	try {
		const r = await del(base, { endpoint: 'https://push.example/nonexistent' });
...
// --- DELETE — 있는 endpoint 는 파일에서 제거 ---
{
	...
	try {
		...
		const r = await del(base, { endpoint: 'https://push.example/endpoint-a' });
		eq('존재 endpoint DELETE → 204', 204, r.status);
		const store = await loadStoreFile(dir);
		eq('파일에서 endpoint 사라짐', undefined, store.subscriptions['https://push.example/endpoint-a']);
	} finally {
		await close();
		await rmDir(dir);
	}
...
// --- DELETE — sentLog 프리픽스도 정리된다 ---
{
	...
	try {
		...
		raw.sentLog = { 'https://push.example/endpoint-a|2026-09-25': '2026-09-25T19:00:00.000Z' };
		...
		eq(
			'sentLog 프리픽스 항목 사라짐',
			undefined,
			after.sentLog['https://push.example/endpoint-a|2026-09-25']
		);
	} finally {
		await close();
		await rmDir(dir);
